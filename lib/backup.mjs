...
export function isBackupPath(p) {
  const normalized = path.resolve(String(p || "")).toLowerCase().split(path.sep).join("/");
  return normalized.includes("/" + BACKUP_SUBDIR.toLowerCase() + "/");
}

export function restoreBackup(backupPath, targetPath) {
  if (!isBackupPath(backupPath)) {
    throw new Error(
      `복원 실패: 백업 파일 위치가 아니에요 — 안전을 위해 .sodamcontext/backups 폴더 안의 파일만 복원할 수 있어요 (${backupPath})`
    );
  }
