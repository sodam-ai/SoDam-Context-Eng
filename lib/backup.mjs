...
// ISO 타임스탬프 (파일명에 안전한 형식: 콜론·마침표→하이픈, 밀리초까지 포함)
// ★수정(07_SECURITY §2.4 대조 감사로 발견): 예전엔 초 단위였다 — 같은 파일을 1초 안에
// 두 번 처방하면 두 번째 백업이 첫 번째를 파일명 충돌로 조용히 덮어썼다(요구사항 "충돌 방지" 위반).
function isoStamp() {
  return new Date().toISOString().replace(/[:.]/g, "-").replace(/Z$/, "");
}

// stamp가 겹쳐도(같은 밀리초 내 재처방 등 극히 드문 경우) 절대 덮어쓰지 않도록 번호를 붙인다.
// export: backup.test.mjs가 타이밍에 좌우되지 않는 결정적 충돌 검증을 위해 직접 호출한다.
export function uniqueBackupPath(backupDir, base, stamp) {
  let candidate = path.join(backupDir, `${base}.${stamp}.bak`);
  for (let n = 2; existsSync(candidate); n++) {
    candidate = path.join(backupDir, `${base}.${stamp}-${n}.bak`);
  }
  return candidate;
}
...
function backupLocal(filePath, backupDir) {
  ...
  const backupPath = uniqueBackupPath(backupDir, base, stamp);
...
export function restoreBackup(backupPath, targetPath) {
  if (!isBackupPath(backupPath, targetPath)) {
    // ★수정(07_SECURITY 대조 감사로 발견): backupFile()은 Harness가 있으면 백업을 위임하지만,
    // 이 함수는 안전을 위해 항상 자체 백업 폴더 안의 파일만 신뢰한다(임의 경로를 Harness에
    // 그대로 넘기면 이 함수가 원래 막던 "임의 파일 읽기" 위험이 다른 경로로 재발할 수 있어
    // 의도적으로 위임하지 않음 — [19] 테스트 참고). 대신 원인을 짐작할 수 있게 안내만 붙인다.
    const hint = harnessAvailable()
      ? " (Harness로 만든 백업이라면 이 도구가 아니라 Harness 자체 명령으로 복구해 주세요)"
      : "";
    throw new Error(
      `복원 실패: 백업 파일 위치가 아니에요 — 안전을 위해 이 파일의 진짜 백업 폴더 안의 파일만 복원할 수 있어요 (${backupPath})${hint}`
    );
  }
