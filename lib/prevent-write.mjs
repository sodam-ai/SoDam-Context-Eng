...
export function evaluateWrite(toolInput, rulesDir) {
  ...
  if (target === "claude") {
    ...
    if (lines >= thresholds.claudeMd.lines_high) {
      return {
        decision: "deny",
        reason: `설명서가 ${lines}줄이 돼요(상한 ${thresholds.claudeMd.lines_high}줄). 너무 길면 AI가 오히려 헷갈려요. 내용을 줄여 주세요.`,
      };
    }
    ...
    if (bytes >= thresholds.agentsMd.bytes_limit) {
      return {
        decision: "deny",
        reason: `코덱스 설명서가 ${bytes}바이트가 돼요(한도 ${thresholds.agentsMd.bytes_limit}바이트). 한글은 글자당 3바이트라 금방 찰 수 있어요. 내용을 줄여 주세요.`,
      };
    }
