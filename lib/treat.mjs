...
// ★T1 안전: 제거 예정 줄의 원문을 removedItems에 담기 전, 비밀키로 보이면 상수 문구로
// 치환한다(scan-secrets.mjs의 masked 표시 방식과 동일한 "부분추출 아닌 통줄 치환" 원칙).
// removedItems는 formatPreviewSummary() 등 사람에게 보여주는 경로로 흘러갈 수 있으므로,
// 이 시점에 마스킹해두면 이후 어떤 소비자가 생기더라도 원문이 새지 않는다.
const SECRET_LINE_PLACEHOLDER = "[비밀키로 의심되는 줄 — 확인을 위해 가려짐]";

function maskIfSecret(line, patterns) {
  if ((patterns || []).some((p) => p.re.test(line))) return SECRET_LINE_PLACEHOLDER;
  return line;
}
 ...
 */
function deduplicateLines(lines, keywords, patterns) {
  const seen = new Set();
  const result = [];
  const removed = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (seen.has(trimmed)) {
      removed.push({ lineNum: i + 1, content: maskIfSecret(line, patterns), rule: "duplicate" });
    } else {
      seen.add(trimmed);
      result.push(line);
    }
 ...
 * @param {Array} [patterns] loadSecretPatterns() 결과 — 있으면 removedItems 원문 마스킹에 사용(T1)
 * @returns {{
 ...
 * }}
 */
export function generateTreatPreview(content, keywords, patterns) {
  const originalLines = content.split("\n");

  // Rule 1: 중복 줄 제거
  const step1 = deduplicateLines(originalLines, keywords, patterns);
