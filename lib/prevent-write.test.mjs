...
// ── ★C-G2(2026-08-31 실측 발견·수정): 예시/플레이스홀더 오탐 방지 ──────
// 실제로 재현했던 결함: AWS 공식 문서가 튜토리얼 전역에서 쓰는 표준 예시 키
// (AKIAIOSFODNN7EXAMPLE)까지 '확정 비밀키'로 오탐해 정상 문서 작성을 deny로 막고 있었다.
test("★C-G2: AWS 공식 EXAMPLE 플레이스홀더 키 → allow(진짜 비밀키 아님)", () => {
  const j = (...p) => p.join("");
  const content = `# 규칙\nAWS 키 형식 예시: ${j("AK", "IA", "IOSF", "ODNN", "7", "EXAMPLE")}\n`;
  const r = evaluateWrite({ file_path: "/x/CLAUDE.md", content }, rulesDir);
  assert.equal(r.decision, "allow");
});

test("★C-G2: x/X 반복 플레이스홀더(sk-ant-xxxx..., ghp_XXXX...) → allow", () => {
  const j = (...p) => p.join("");
  const content1 = `API 키는 ${j("sk", "-ant-", "x".repeat(24))} 형식입니다.`;
  const content2 = `${j("gh", "p_", "X".repeat(38))} 를 발급받아 넣으세요.`;
  assert.equal(evaluateWrite({ file_path: "/x/CLAUDE.md", content: content1 }, rulesDir).decision, "allow");
  assert.equal(evaluateWrite({ file_path: "/x/CLAUDE.md", content: content2 }, rulesDir).decision, "allow");
});

test("★C-G2 회귀 확인(중요): 플레이스홀더 마커 없는 진짜 형태 가짜 키는 여전히 deny", () => {
  // FAKE_KEY(=sk-ant-+A반복)엔 EXAMPLE/YOUR_HERE/REDACTED도 없고 x/X 6연속도 없다 —
  // allowlist가 과도하게 넓어져 진짜 유출까지 눈감아주면 안 된다는 걸 확인하는 음성 대조군.
  const content = `api_key = ${FAKE_KEY}\n`;
  const r = evaluateWrite({ file_path: "/x/CLAUDE.md", content }, rulesDir);
  assert.equal(r.decision, "deny", "allowlist가 과도하게 넓어져 진짜 유출까지 통과시킴(회귀)");
});
...
// ── ★C-G3(2026-09-01 실측 발견·수정): 크기초과 deny가 막다른 벽이면 안 됨 ──────
// 발견: "내용을 줄여 주세요"까지만 안내하고, 이 도구 자신의 처방(treat) 명령을
// 알려주지 않아 초보자가 "뭘 어떻게 줄이나"에서 막혔음(CHECKPOINT.md C-G3).
test("★C-G3: CLAUDE.md 줄수초과 deny 안내에 처방 명령 경로가 포함됨", () => {
  const content = Array.from({ length: 320 }, (_, i) => `줄 ${i}`).join("\n");
  const r = evaluateWrite({ file_path: "/x/CLAUDE.md", content }, rulesDir);
  assert.ok(r.reason.includes("/sodam-context:treat"), "막다른 벽(치료 경로 안내 없음)");
});
...
test("★C-G3: AGENTS.md 바이트초과 deny 안내에도 처방 명령 경로가 포함됨", () => {
  const content = "가".repeat(11000);
  const r = evaluateWrite({ file_path: "/x/AGENTS.md", content }, rulesDir);
  assert.ok(r.reason.includes("/sodam-context:treat"), "막다른 벽(치료 경로 안내 없음)");
});
