...
test("[1b] ★회귀(2026-08-20 검증 라운드): checkup 이력에 비밀키로 인한 감점도 findingIds에 기록됨", () => {
  // 재현: checkupFile()이 healthScore.breakdown.mediumCount/highCount는 secret.findings를
  // 포함해 계산하지만, checkup-history.mjs로 넘기는 findingIds는 rules/suspects만 매핑해
  // secret.findings를 빠뜨렸었다 — 감점은 됐는데 원인이 findingIds에 하나도 안 남는 불일치.
  const histPath = path.join(dir, ".sodamcontext", "checkup-history.jsonl");
  assert.ok(existsSync(histPath), "이력 파일이 생성되지 않음");
  const lines = readFileSync(histPath, "utf8").split("\n").filter((l) => l.trim());
  const last = JSON.parse(lines[lines.length - 1]);
  assert.ok(last.findingIds.includes("secret"), "비밀키로 인한 감점이 findingIds에 안 남음");
  assert.ok(!JSON.stringify(last).includes(FAKE_KEY), "★이력 파일에 비밀키 원문(T1 위반)");
});
...
test("[2] backup: 백업 파일 생성 (+ local이면 .gitignore 자동 등록)", () => {
  ...
  if (r.method === "local") {
    ...
    // ★회귀(2026-08-19 검증 라운드): method가 local이면 Harness 안내(note)가 섞여 나오면 안 됨 —
    // 이전엔 암묵적으로만 참이었던 걸(모든 테스트가 항상 local이라 note가 한 번도 안 보임)
    // 명시 단언으로 고정.
    assert.ok(!("note" in r), "local 백업인데 Harness 안내(note)가 섞여 나옴");
  }
});
...
test("[10b] ★회귀(2026-08-20 검증 라운드): apply/preview 대상 파일 없음 → 쉬운 한국어 메시지, Node 원본 에러(ENOENT) 미노출", () => {
  // 재현 배경: preview/apply만 backup/restore/deep-scan과 달리 파일 존재 확인이 없어서,
  // 없는 파일을 넘기면 readFileSync가 던진 원본 Node 에러("ENOENT: no such file or
  // directory, open '...'")가 그대로 노출됐다 — 완전 초보자용 도구인데 개발자 전용 에러
  // 문구가 그대로 보이는 건 01_PRD.md의 "한국어·비개발자 표현" 원칙 위반.
  const missing = path.join(dir, "__없는파일__.md");
  const rApply = runCLIRaw([missing, "--action", "apply"], dir);
  assert.equal(rApply.stdout.error, "처방할 파일을 찾을 수 없어요");
  assert.ok(!JSON.stringify(rApply).includes("ENOENT"), "apply가 Node 원본 에러를 그대로 노출함");

  const rPreview = runCLIRaw([missing, "--action", "preview"], dir);
  assert.equal(rPreview.status, 1, "preview exit code 불일치");
  assert.equal(rPreview.stdout.ok, false);
  assert.equal(rPreview.stdout.error, "처방할 파일을 찾을 수 없어요");
  assert.ok(!JSON.stringify(rPreview).includes("ENOENT"), "preview가 Node 원본 에러를 그대로 노출함");
});
...
// ── [22] ★회귀: apply가 backup 액션 없이 직접 호출돼도 이제 자동으로 백업을 만듦 ──
// QA(2026-07-27)에서 발견: apply 액션은 commands/treat.md의 "백업 먼저" 프롬프트 지시에만
// 기대고 있었고, CLI 코드 자체는 backupFile()을 호출하지 않았다 — --action backup 단계 없이
// --action apply를 직접 호출하면 원본이 백업 없이 그대로 덮어써졌다(02_DATA_MODEL.md "처방은
// 반드시 백업 후에만 적용" 불변 규칙 위반, 08_DEEP A3 "프롬프트 지시는 준수를 보장하지 않는다"
// 원칙과도 충돌). apply 안에 backupFile() 호출을 추가해 코드 레벨로 강제.
test("[22] ★apply: backup 액션을 먼저 호출하지 않아도 자동으로 백업을 생성함", () => {
  const soloDir = mkdtempSync(path.join(os.tmpdir(), "sdc-cli-apply-autobackup-"));
  try {
    const f = path.join(soloDir, "CLAUDE.md");
    writeFileSync(f, "중복줄\n중복줄\n절대 force push 금지.\n", "utf8");
    // 의도적으로 backup 액션을 건너뛰고 바로 apply만 호출
    const r = runCLI([f, "--action", "apply"], soloDir);
    assert.equal(r.ok, true);
    assert.ok(r.backupPath, "apply 응답에 backupPath가 없음 — 백업이 자동 생성되지 않음");
    assert.ok(existsSync(r.backupPath), "backupPath가 가리키는 백업 파일이 실제로 없음");
    const backupContent = readFileSync(r.backupPath, "utf8");
    assert.ok(backupContent.includes("중복줄\n중복줄"), "백업 내용이 처방 전 원본과 다름");
  } finally {
    rmSync(soloDir, { recursive: true, force: true });
  }
});

// ── [23]~[25] deep-scan (Phase 3 "AI 판단 강화") ────────────────────────────
// 2026-08-02 신설. checkup과 달리 파일 내용을(마스킹 후) 그대로 반환하므로 T1·경로안전 별도 검증.

test("[23] deep-scan: 정상 흐름 — 비밀키 없는 파일 → 그대로 통과 + hintlessChecks 5종 반환", () => {
  const soloDir = mkdtempSync(path.join(os.tmpdir(), "sdc-cli-deepscan-normal-"));
  try {
    const f = path.join(soloDir, "CLAUDE.md");
    const content = "항상 테스트를 먼저 작성하라.\n테스트는 나중에 작성하라.\n";
    writeFileSync(f, content, "utf8");
    const r = runCLI([f, "--action", "deep-scan"], soloDir);
    assert.equal(r.ok, true);
    assert.equal(r.maskedText, content, "비밀키 없는데도 내용이 원본과 달라짐");
    assert.equal(r.maskedLineCount, 0);
    const ids = r.hintlessChecks.map((c) => c.id).sort();
    assert.deepEqual(ids, ["blind_references", "bloat", "conflicting", "no_examples", "re-scope-guard"]);
  } finally {
    rmSync(soloDir, { recursive: true, force: true });
  }
});

test("[23] deep-scan: --action 신설이 기존 checkup 경로를 바꾸지 않음(회귀 없음)", () => {
  const r = runCLI([file], dir);
  assert.equal(r.ok, true);
  assert.ok(Array.isArray(r.aiSuspectQueue) && r.aiSuspectQueue.length === 5, "기존 aiSuspectQueue 동작이 바뀜");
});

test("[24] ★deep-scan T1: 비밀키 포함 파일 → maskedText에 원문 없음 + 해당 줄 가려짐", () => {
  const r = runCLI([file, "--action", "deep-scan"], dir);
  assert.equal(r.ok, true);
  assert.ok(r.maskedLineCount >= 1, "비밀키 있는 줄인데 마스킹 카운트 0");
  assert.ok(!r.maskedText.includes(FAKE_KEY), "★T1 위반: maskedText에 비밀키 원문이 남음");
  assert.ok(r.maskedText.includes("가려짐"), "마스킹 표시 문구가 없음");
});

test("[25] ★deep-scan: 자격증명 폴더(.ssh) 대상 지정 → 거부(존재 여부와 무관하게 차단)", () => {
  const credLikePath = path.join(os.homedir(), ".ssh", "id_rsa_존재안함_QA전용");
  const r = runCLIRaw([credLikePath, "--action", "deep-scan"], dir);
  assert.equal(r.stdout.ok, false, "자격증명 폴더 대상인데 deep-scan이 허용됨(위험)");
  assert.equal(r.status, 1, "exit code 불일치");
  assert.ok(r.stdout.error.includes("위치"), "이유 메시지 없음");
});

// ── [26] ★07_SECURITY §2.4: 같은 파일을 연달아 백업해도 이전 백업을 덮어쓰지 않음 ──
// 대조 감사로 발견: 예전 타임스탬프는 초 단위라 같은 초 안에 두 번 처방하면 두 번째 백업이
// 첫 번째를 파일명 충돌로 조용히 덮어썼다(07_SECURITY.md §2.4 "충돌 방지" Must 위반).
// 밀리초 단위로 바꾸고 그래도 겹치면 번호를 붙이도록 고쳤다 — 실제 CLI 연속 호출로 검증.
// (정확히 같은 밀리초로 겹치는 극단적 케이스는 backup.test.mjs가 stamp를 고정해 결정적으로 검증)
test("[26] ★backup: 같은 파일을 연달아 두 번 백업해도 서로 다른 백업 파일로 둘 다 보존됨", () => {
  const soloDir = mkdtempSync(path.join(os.tmpdir(), "sdc-cli-backup-collision-"));
  try {
    const f = path.join(soloDir, "CLAUDE.md");
    writeFileSync(f, "첫 번째 내용\n", "utf8");
    const r1 = runCLI([f, "--action", "backup"], soloDir);
    assert.equal(r1.ok, true);
    writeFileSync(f, "두 번째 내용\n", "utf8");
    const r2 = runCLI([f, "--action", "backup"], soloDir);
    assert.equal(r2.ok, true);
    assert.notEqual(r1.backupPath, r2.backupPath, "두 백업의 경로가 같음 — 충돌 시 덮어쓰기 위험");
    assert.ok(existsSync(r1.backupPath), "첫 번째 백업이 사라짐(덮어써짐)");
    assert.ok(existsSync(r2.backupPath), "두 번째 백업 파일 없음");
    assert.equal(readFileSync(r1.backupPath, "utf8"), "첫 번째 내용\n", "첫 번째 백업 내용이 훼손됨(덮어써짐)");
    assert.equal(readFileSync(r2.backupPath, "utf8"), "두 번째 내용\n", "두 번째 백업 내용이 다름");
  } finally {
    rmSync(soloDir, { recursive: true, force: true });
  }
});

// ── [27] ★2026-09-01 실측 발견: CLAUDE.md/AGENTS.md가 아닌 파일은 freshness·history 미기록 ──
// 실제로 로컬 .sodamcontext/checkup-history.jsonl에 소스 코드 파일(.mjs)을 실수로 검진한
// 오염 데이터 3건이 발견됐다(Phase 3 건강점수 정식화용 실사용 데이터를 왜곡할 위험).
// 원인: checkupFile()이 대상 파일 종류를 안 가리고 아무 경로나 freshness·history에 기록했음.
test("[27] ★checkup: 대상 아닌 파일(.js)은 검진은 되지만 .sodamcontext/history·freshness엔 기록 안 됨", () => {
  const soloDir = mkdtempSync(path.join(os.tmpdir(), "sdc-cli-nontarget-"));
  try {
    const f = path.join(soloDir, "some-source.js");
    writeFileSync(f, "// 그냥 소스 코드\nconsole.log('hi');\n", "utf8");
    const r = runCLI([f], soloDir);
    assert.equal(r.ok, true, "대상 아닌 파일이어도 핵심 검진 자체는 여전히 동작해야 함");
    assert.deepEqual(r.freshness, { previousCheckupAt: null, daysSinceLastCheckup: null, stale: false });
    const histPath = path.join(soloDir, ".sodamcontext", "checkup-history.jsonl");
    const lastPath = path.join(soloDir, ".sodamcontext", "last-checkup.json");
    assert.ok(!existsSync(histPath), "대상 아닌 파일인데 이력 파일이 생성됨(오염 위험)");
    assert.ok(!existsSync(lastPath), "대상 아닌 파일인데 freshness 기록 파일이 생성됨(오염 위험)");
  } finally {
    rmSync(soloDir, { recursive: true, force: true });
  }
});

test("[27b] ★대조군: 같은 폴더에서 CLAUDE.md는 평소대로 이력·freshness가 정상 기록됨(회귀 방지)", () => {
  const soloDir = mkdtempSync(path.join(os.tmpdir(), "sdc-cli-target-"));
  try {
    const f = path.join(soloDir, "CLAUDE.md");
    writeFileSync(f, "한국어로 답해주세요.\n", "utf8");
    const r = runCLI([f], soloDir);
    assert.equal(r.ok, true);
    const histPath = path.join(soloDir, ".sodamcontext", "checkup-history.jsonl");
    const lastPath = path.join(soloDir, ".sodamcontext", "last-checkup.json");
    assert.ok(existsSync(histPath), "대상 파일(CLAUDE.md)인데 이력이 기록 안 됨(과차단)");
    assert.ok(existsSync(lastPath), "대상 파일(CLAUDE.md)인데 freshness가 기록 안 됨(과차단)");
  } finally {
    rmSync(soloDir, { recursive: true, force: true });
  }
});
