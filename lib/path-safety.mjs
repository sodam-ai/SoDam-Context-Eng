...
// ★2026-09-01 CI 실측 발견: opts.platform으로 실제 실행 OS와 다른 플랫폼을 시뮬레이션할 때
// (테스트 목적), 아래에서 계속 ambient `path`(OS 실제값 반영)를 쓰면 절대경로 판정·join이
// 시뮬레이션 대상과 어긋난다(Windows 경로를 Linux에서 시뮬레이션하면 path.isAbsolute가
// false로 나와 전혀 다른 경로로 resolve됨 — CI를 ubuntu-latest에서 처음 돌려 발견).
// platform에 맞는 path.win32/path.posix를 명시적으로 골라 일관되게 쓴다.
...
function isSensitiveAgainst(a, home, platform, sep) {
  ...
  for (const sub of CRED_SUBDIRS) {
    const sd = toComparable(p.join(home, sub), platform);
...
export function isSensitiveWritePath(targetPath, opts = {}) {
  ...
  try {
    const p = pathFor(platform);
    abs = p.isAbsolute(targetPath) ? targetPath : p.resolve(targetPath);
  } catch {
