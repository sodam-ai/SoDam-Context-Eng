...
// opts: { homeDir, gitRoot, codexHome } — 전부 선택.
//   codexHome 미지정(undefined) 시 기존 동작(<homeDir>/.codex)을 그대로 유지(회귀 방지).
//   codexHome 지정 시 그 경로 자체를 글로벌 디렉터리로 사용 — CODEX_HOME 환경변수가
//   설정되면 실제 코덱스는 <home>/.codex가 아니라 CODEX_HOME을 그대로 글로벌 홈으로 쓰므로
//   (config.toml도 CODEX_HOME 바로 아래에 위치, `codex doctor`로 실측 확인) 하드코딩된
//   <home>/.codex만 보면 CODEX_HOME 사용자에겐 조용히 엉뚱한 파일을 검진하게 됨.
...
export function collectCodexChain(startDir, opts) {
  ...
  // 1) 글로벌 — codexHome(=CODEX_HOME) 지정 시 그 경로, 아니면 기존 <home>/.codex — 가장 먼저(가장 약함)
  const globalDir = o.codexHome !== undefined ? o.codexHome : (home ? path.join(home, ".codex") : null);
  if (globalDir) {
    for (const n of FALLBACK_NAMES) {
      const f = path.join(globalDir, n);
  ...
  // B4: config.toml 감지 — 글로벌(CODEX_HOME) 디렉터리도 스캔 대상에 포함(기존엔 프로젝트 상위트리만 봐서 실제 config.toml 위치를 놓쳤음)
  const configOverride = detectConfigToml(globalDir ? [globalDir, ...dirs] : dirs);
...
if (invokedDirect) {
  ...
  // CODEX_HOME 환경변수가 설정된 실제 코덱스 환경에서는 그 경로가 진짜 글로벌 홈(<home>/.codex 아님).
  const codexHome = process.env.CODEX_HOME || undefined;
  const chain = collectCodexChain(path.resolve(process.cwd(), start), { gitRoot, codexHome });
