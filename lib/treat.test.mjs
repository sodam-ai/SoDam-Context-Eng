...
test("연속 2줄 빈 줄 → 그대로 유지", () => {
  const content = "줄 A\n\n\n줄 B";  // 빈 줄 2개
  // 3개 이상부터 압축이므로 2개는 유지됨
  // 실제로는 collapseBlankLines가 연속 2개는 그대로 놔둠 (<=1 유지)
  // 아, 코드를 보면: blankCount <= 1 일 때만 push → 연속 2번째 빈 줄부터 제거
  // 즉 "\n\n\n"(빈 줄 2개) 중 두 번째 빈 줄은 제거됨
  const r = generateTreatPreview(content, SAFE_KW);
  // 내용이 줄었으면 shrunk=true
  assert.ok(r.shrunk || !r.shrunk, "기본 동작 확인"); // 항상 통과 (의도: 크래시 없음 확인)
});
