...
자동화한 것뿐이에요. **이 저장소는 공개(PUBLIC) 저장소지만, 이 스크립트 파일 하나만 골라 받을 수 있는
별도 링크는 없어서, 스크립트를 처음 손에 넣으려면 딱 한 번은 아래 1단계(`git clone`)를 직접 해야
합니다.**
...
| 규칙으로 못 잡는 문제까지 깊게 검진(선택) | "깊은 검진 해줘" 또는 "모순 있는지 직접 읽고 판단해줘" |
...
SoDamContext는 내부적으로 Node.js를 사용합니다(2026-07-18부터 스킬(SKILL.md) 방식은 폐지되고
`commands/*.md`로 통합됐습니다 — 코덱스는 이 파일들을 `AGENTS.md`를 통해 자연어로 찾아 읽습니다).

```
Codex → AGENTS.md·commands/*.md 읽기 → node -e "..." 실행 → 결과 분석 → 리포트 출력
```

> ⚠️ **경로 주의(코덱스)**: `commands/*.md` 안의 명령은 `${CLAUDE_PLUGIN_ROOT}/lib/…`로 적혀 있는데, 이건 **클로드코드 전용** 변수입니다.
