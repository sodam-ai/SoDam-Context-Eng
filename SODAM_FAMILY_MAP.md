...
> **⚠️ 참조용 사본**: 정본은 별도 SoDam_Family 프로젝트 저장소에 있습니다(PRD 09 §9). 이 파일은
> Context-Eng용 사본이며, 수정 시 정본도 함께 수정할 것.
> ★2026-08-13 정정: 이 문서가 커밋된 이후 저장소가 PUBLIC으로 전환됨에 따라, 내부 진행 상태를
> 담았던 구 §6 "버전 현황" 표(형제별 AI 작업 상태·블로커, 2026-06-28 스냅샷)를 제거했습니다 —
> 이 파일의 목적(§1~§5 역할 경계·API 계약·버전 호환 규칙)과 무관한 내부 운영 정보였고, 공개
> 저장소에 남겨둘 근거가 없었습니다(PRD `07_SECURITY.md`·`10_LICENSE_LEGAL.md`의 "내부 자료
> 무단노출 금지" 원칙). 최신 진행 상태가 필요하면 각 프로젝트의 (비공개) CHECKPOINT.md를 확인하세요.
...
> 최종 수정: 2026-08-13 (§6 "버전 현황" 표 제거 — 위 "2026-08-13 정정" 참조)
